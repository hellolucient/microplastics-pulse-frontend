...
const LatestNewsPage: React.FC = () => {
    ...
    const fetchCurrentPageFromApi = async () => {
      ...
      try {
        ...
        if (searchTerm.trim()) {
          ...
          response = await axios.get(`${BACKEND_URL}/api/latest-news/search?q=${encodeURIComponent(searchTerm)}&page=${currentPage}&limit=${storiesPerPage}`);
        } else {
          // Use regular endpoint when no search term
          response = await axios.get(`${BACKEND_URL}/api/latest-news?page=${currentPage}&limit=${storiesPerPage}`);
        }
  ...
  // No need for client-side filtering since search is done on backend
  const filteredNewsItems = newsItems;

  // Pagination logic - now using backend pagination
  const storiesPerPage = viewMode === 'grid' ? STORIES_PER_PAGE_GRID : STORIES_PER_PAGE_LIST;

  // Get stories for current page - backend already handles pagination
  ...
  if (viewMode === 'grid') {
    if (currentPage === 1) {
      ...
      secondaryStories = filteredNewsItems.slice(1);
    } else {
      secondaryStories = filteredNewsItems;
    }
  } else {
    // List view: show all items from backend
    listStories = filteredNewsItems;
  }
        ...
        <>
          ...
          <div className="flex flex-col items-center mt-12 space-y-4">
            ...
            <div className="text-sm text-gray-600">
              Showing {((currentPage - 1) * storiesPerPage) + 1} to {Math.min(currentPage * storiesPerPage, filteredNewsItems.length)} of {totalPages * storiesPerPage} articles
